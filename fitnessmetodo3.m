function fitness = fitnessmetodo3(error)
% Fitness baseado na IAE com punição de erro estacionário e kp e ki fora do
% do valor de ressonância em RP

% tira o transitório incial e começa a calcular a IAE a partir dos
% transitórios de vento
n11 = find(error(:,1)<1.5); n11 = n11(end);
n12 = find(error(:,1)<1.5+0.06); n12 = n12(end);

n21 = find(error(:,1)<2.5); n21 = n21(end);
n22 = find(error(:,1)<2.5+0.06); n22 = n22(end);
...
%Amplitudes
amp_trans_v(1) = min(error(n11:n12,2));
amp_trans_v(2) = max(error(n21:n22,2));

if (any(amp_trans_v > 0.05)) || ((any(amp_trans_v < -0.05))); pen_pss = 100; else pen_pss = 0; end
